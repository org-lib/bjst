...
export class GuiFramework {
    ...
    /*
    在 Babylon.js 中，AdvancedDynamicTexture.getDescendants 是一个方法，用于获取 AdvancedDynamicTexture（高级动态纹理）对象的子控件列表。
    该方法返回一个 Control[] 数组，其中包含了所有直接子控件或符合指定条件的子控件。

    参数说明：

    directDescendantsOnly（可选）：一个布尔值，表示是否只返回直接子控件。默认为 false，即返回所有子控件，包括嵌套子控件。
    predicate（可选）：一个回调函数，用于筛选子控件。只有当回调函数返回 true 时，才将子控件包含在结果中。
    通过调用 AdvancedDynamicTexture.getDescendants 方法，可以方便地获取 AdvancedDynamicTexture 对象下的所有子控件或满足指定条件的子控件。这对于遍历和操作控件层次结构非常有用，例如对所有子控件进行样式设置、事件绑定等操作。

    以下是一个示例：

    import { AdvancedDynamicTexture, Control } from "@babylonjs/gui";

    // 创建 AdvancedDynamicTexture 对象
    const adt = AdvancedDynamicTexture.CreateFullscreenUI("UI");

    // 添加子控件
    const button1 = new BABYLON.GUI.Button("button1");
    adt.addControl(button1);

    const button2 = new BABYLON.GUI.Button("button2");
    button1.addControl(button2);

    const button3 = new BABYLON.GUI.Button("button3");
    adt.addControl(button3);

    // 获取所有子控件
    const allDescendants = adt.getDescendants();
    console.log(allDescendants);
    // 输出：[button1, button2, button3]

    // 获取直接子控件
    const directDescendants = adt.getDescendants(true);
    console.log(directDescendants);
    // 输出：[button1, button3]

    // 获取符合条件的子控件
    const predicateDescendants = adt.getDescendants(false, (control: Control) => control instanceof BABYLON.GUI.Button);
    console.log(predicateDescendants);
    // 输出：[button1, button2, button3]
    在上述示例中，我们创建了一个 AdvancedDynamicTexture 对象，并添加了几个子控件。然后，通过调用 getDescendants 方法，分别获取了所有子控件、直接子控件和符合条件的子控件，并将结果打印到控制台。
    */
    ...
    public static createBottomBar(adt: AdvancedDynamicTexture) {
        let bottomBarLeft: Image = new Image("bottomBarLeft", "/src/assets/UI/bottomBarLeft.svg");
        let bottomBarCenter: Image = new Image("bottomBarCenter", "/src/assets/UI/bottomBarCenter.svg");
        let bottomBarRight: Image = new Image("bottomBarRight", "/src/assets/UI/bottomBarRight.svg");
    ...
    public static createTextPanel(parentGrid: Grid) {
        let textPanelUL: Image = new Image("bottomBarLeft", "/src/assets/UI/textPanelUL.svg");
        let textPanelUC: Image = new Image("bottomBarCenter", "/src/assets/UI/textPanelUC.svg");
        let textPanelUR: Image = new Image("bottomBarRight", "/src/assets/UI/textPanelUR.svg");
        let textPanelCL: Image = new Image("bottomBarLeft", "/src/assets/UI/textPanelCL.svg");
        let textPanelCC: Image = new Image("bottomBarCenter", "/src/assets/UI/textPanelCC.svg");
        let textPanelCR: Image = new Image("bottomBarRight", "/src/assets/UI/textPanelCR.svg");
        let textPanelLL: Image = new Image("bottomBarLeft", "/src/assets/UI/textPanelLL.svg");
        let textPanelLC: Image = new Image("bottomBarCenter", "/src/assets/UI/textPanelLC.svg");
        let textPanelLR: Image = new Image("bottomBarRight", "/src/assets/UI/textPanelLR.svg");
        let grid: Grid = new Grid();
    ...
    public static addButton(label: string, panel: StackPanel): Button {
        var button = Button.CreateImageButton("button", label.toUpperCase(), "/src/assets/UI/menuButton.svg");
